...
const FILE_NAME = "ins"; // don't give .pdf file extension (for people who keeps on adding .pdf file extension even after frequent changing)
const PORT = 4000;
...
export function IncomeSheetGen() {
  ...
  const createAndDownloadPdf = (event) => {
    axios
      .post(`http://localhost:${PORT}/ins/create`, formik.values)
      .then(() => {
        axios
          .get(`http://localhost:${PORT}/ins/get`, { responseType: "blob" })
          .then((res) => {
